...
#TODO: add skipped tests if missing dependencies
...
class TestPDFUtils(unittest.TestCase):
	...
	def test_convert_mac(self):
		import platform
		self.assertEqual(platform.system(), 'Darwin', 'Machine has to be a mac to test')
		...
		try:
			...
			output = subprocess.check_output('file %s' % temppdffile_path, shell=True)
	...
	def test_convert_wkhtmltopdf(self):
		...
		convert_utils.create_PDF = partial(convert_utils.convert_wkhtmltopdf, wkhtmltopdf_path=wkhtmltopdf_path)
	...
	def test_pdktk_write_PDF_encrypted(self):
		...
		convert_utils.write_PDF = partial(convert_utils.pdktk_write_PDF, pdftk_path=pdftk_path)
	...
	def test_reportlab_write_PDF_encrypted(self):
		#Assuming reportlab and rlextra will be installed by default and automatically imported
